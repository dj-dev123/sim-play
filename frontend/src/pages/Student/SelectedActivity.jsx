...
export default function SelectedActivity() {
  ...
  const [previousActivityId, setPreviousActivityId] = useState(null);
  const [isCompleted, setIsCompleted] = useState(false);
  const [loading, setLoading] = useState(true);

  usePageTitle(activity?.title || "Activity");

  useEffect(() => {
    setLoading(true);
    api.get(`/student/activities/${id}`).then(({ data }) => {
      setActivity(data.activity);
      setNextActivityId(data.nextActivityId);
      setPreviousActivityId(data.previousActivityId);
      ...
      <div className="min-h-screen bg-[#cfe9c8] flex justify-center px-4 py-10">
        <div className="relative w-full max-w-5xl bg-[#dff0d8] rounded-3xl shadow-xl p-6 sm:p-10">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4 pt-6 border-t border-[#b6d7a8]">
            ...
            {previousActivityId ? (
              <Link
                to={paths.studentActivity(previousActivityId)}
            ...
            {isCompleted ? (
              ...
              <Link
                to={paths.studentActivity(nextActivityId)}
